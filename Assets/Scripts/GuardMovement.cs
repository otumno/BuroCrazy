...
public class GuardMovement : MonoBehaviour
{
    ...
    public bool ShouldWork(bool isDayTime)
    {
        if (!isOnDuty && (assignedShift == Shift.Universal || (isDayTime && assignedShift == Shift.Day) || (!isDayTime && assignedShift == Shift.Night)))
        {
            return true;
        }
        return false;
    ...
    public void EndShift() { if(isOnDuty) { if(currentAction != null) StopCoroutine(currentAction); currentAction = StartCoroutine(EndShiftRoutine()); } }
    ...
    public void ReturnToPatrol() { if(isOnDuty && currentState == GuardState.OnBreak) { if(currentAction != null) StopCoroutine(currentAction); currentAction = StartCoroutine(ReturnToPatrolRoutine()); } }
    ...
    private IEnumerator StartShiftRoutine() { isOnDuty = true; if(startShiftSound != null && homePointWaypoint != null) AudioSource.PlayClipAtPoint(startShiftSound, homePointWaypoint.transform.position); yield return StartCoroutine(ReturnToPatrolRoutine()); }
    private IEnumerator EndShiftRoutine() { isOnDuty = false; SetState(GuardState.OffDuty); if (homePointWaypoint != null) { yield return StartCoroutine(MoveToTarget(homePointWaypoint.transform.position, GuardState.OffDuty)); if(endShiftSound != null) AudioSource.PlayClipAtPoint(endShiftSound, homePointWaypoint.transform.position); } else { Debug.LogWarning($"HomePointWaypoint не назначен для {gameObject.name}. Охранник останется на месте."); } currentAction = null; }
    ...
    { 
        ...
        // Теперь охранник будет ждать в цикле, пока период "Обед" не закончится
        while (ClientSpawner.CurrentPeriodName != null && ClientSpawner.CurrentPeriodName.ToLower().Trim() == "обед")
        {
            yield return null; // Ждем один кадр
        }

        // Когда обед закончился, ClientSpawner сам вызовет ReturnToPatrol()
    }
    ...
    private IEnumerator CatchThiefRoutine(ClientPathfinding target) { currentChaseTarget = target; SetState(GuardState.ChasingThief); agentMover.moveSpeed *= chaseSpeedMultiplier; StartShouting(); Physics2D.IgnoreLayerCollision(guardLayer, clientLayer, true); while(currentChaseTarget != null && Vector2.Distance(transform.position, currentChaseTarget.transform.position) > catchDistance) { agentMover.SetPath(BuildPathTo(currentChaseTarget.transform.position)); yield return new WaitForSeconds(0.5f); } if (currentChaseTarget != null) { yield return StartCoroutine(EscortThiefToCashier()); } else { GoBackToDuties(); } }
    private IEnumerator EvictRoutine(ClientPathfinding target) { currentChaseTarget = target; SetState(GuardState.Evicting); while(currentChaseTarget != null && Vector2.Distance(transform.position, currentChaseTarget.transform.position) > catchDistance) { agentMover.SetPath(BuildPathTo(currentChaseTarget.transform.position)); yield return new WaitForSeconds(0.5f); } if (currentChaseTarget != null) { SetState(GuardState.Talking); agentMover.Stop(); currentChaseTarget.Freeze(); transform.position = (Vector2)currentChaseTarget.transform.position + Random.insideUnitCircle * 0.6f; if (reprimandSound != null) AudioSource.PlayClipAtPoint(reprimandSound, transform.position); yield return new WaitForSeconds(talkTime); if (currentChaseTarget != null) { currentChaseTarget.stateMachine.GetHelpFromIntern(ClientSpawner.Instance.exitWaypoint); } } GoBackToDuties(); }
    private IEnumerator TalkToClient() { SetState(GuardState.Talking); StopShouting(); ClientPathfinding clientToCalm = currentChaseTarget; if (clientToCalm == null) { GoBackToDuties(); yield break; } clientToCalm.Freeze(); agentMover.Stop(); transform.position = (Vector2)clientToCalm.transform.position + Random.insideUnitCircle * 0.6f; if (reprimandSound != null) AudioSource.PlayClipAtPoint(reprimandSound, transform.position); yield return new WaitForSeconds(talkTime); if(clientToCalm != null) { clientToCalm.UnfreezeAndRestartAI(); if (Random.value < 0.5f) { clientToCalm.CalmDownAndReturnToQueue(); } else { clientToCalm.CalmDownAndLeave(); } if(ClientQueueManager.Instance != null) ClientQueueManager.Instance.dissatisfiedClients.Remove(clientToCalm); } currentStress += stressGainPerViolator; GoBackToDuties(); }
    private IEnumerator EscortThiefToCashier() { SetState(GuardState.EscortingThief); StopShouting(); ClientPathfinding thief = currentChaseTarget; if (thief != null) { thief.Freeze(); agentMover.Stop(); transform.position = (Vector2)thief.transform.position + Random.insideUnitCircle * 0.6f; if (reprimandSound != null) AudioSource.PlayClipAtPoint(reprimandSound, transform.position); yield return new WaitForSeconds(talkTime); LimitedCapacityZone cashierZone = ClientSpawner.GetCashierZone(); if (cashierZone != null) { thief.stateMachine.StopAllActionCoroutines(); thief.stateMachine.SetGoal(cashierZone.waitingWaypoint); thief.stateMachine.SetState(ClientState.MovingToGoal); } } currentStress += stressGainPerViolator; GoBackToDuties(); }
    private void GoBackToDuties() { if(currentState == GuardState.Chasing || currentState == GuardState.ChasingThief) {agentMover.moveSpeed /= chaseSpeedMultiplier;} if (GuardManager.Instance != null && currentChaseTarget != null) { GuardManager.Instance.ReportTaskFinished(currentChaseTarget); } Physics2D.IgnoreLayerCollision(guardLayer, clientLayer, false); StopShouting(); currentChaseTarget = null; currentAction = null; if (ClientSpawner.Instance != null && isOnDuty) { ClientSpawner.Instance.SendMessage("ManageGuardShifts", ClientSpawner.CurrentPeriodName.ToLower().Trim(), SendMessageOptions.DontRequireReceiver); } }
    private IEnumerator MoveToTarget(Vector2 targetPosition, GuardState stateOnArrival) { var path = BuildPathTo(targetPosition); if (path.Count == 0 && Vector2.Distance(transform.position, targetPosition) > 0.5f) { Debug.LogWarning($"Охранник {gameObject.name} не смог построить путь к {targetPosition}. Проверьте доступность точки."); SetState(stateOnArrival); yield break; } agentMover.SetPath(path); yield return new WaitUntil(() => !agentMover.IsMoving()); SetState(stateOnArrival); }
    ...
    public string GetStatusInfo() { switch (currentState) { case GuardState.StressedOut: return "СОРВАЛСЯ!"; case GuardState.Patrolling: return $"Патрулирует. Цель: {currentPatrolTarget?.name}"; case GuardState.OnPost: return $"На посту"; case GuardState.Chasing: return $"Преследует: {currentChaseTarget?.name}"; case GuardState.Talking: return $"Разговаривает с: {currentChaseTarget?.name}"; case GuardState.WaitingAtWaypoint: return $"Ожидает на точке: {currentPatrolTarget?.name}"; case GuardState.OffDuty: return "Смена окончена"; case GuardState.ChasingThief: return $"Ловит воришку: {currentChaseTarget?.name}"; case GuardState.EscortingThief: return $"Ведет в кассу: {currentChaseTarget?.name}"; case GuardState.Evicting: return $"Выпроваживает: {currentChaseTarget?.name}"; default: return currentState.ToString(); } }
