...
public class ClientNotification : MonoBehaviour
{
    ...
    public void UpdateNotification() { if (notificationText == null || parent == null || parent.stateMachine == null) return; string stateText = GetStateText(); notificationText.text = stateText; notificationText.color = GetStateColor(); if (queueNumberText != null) { ClientState cs = parent.stateMachine.GetCurrentState(); Waypoint goal = parent.stateMachine.GetCurrentGoal(); bool isGoingToReg = (cs == ClientState.MovingToGoal && goal != null && goal.gameObject == parent.queueManager.GetRegistrationZone()); bool showQueue = (cs == ClientState.AtWaitingArea || cs == ClientState.AtRegistration || isGoingToReg) && queueNumber >= 0; queueNumberText.text = showQueue ? queueNumber.ToString() : ""; } }
    ...
    private string GetStateText()
    {
        ...
        switch (state) { case ClientState.Spawning: return "?"; case ClientState.AtWaitingArea: return "Zz"; case ClientState.AtToilet: return "!"; case ClientState.Confused: return "?"; case ClientState.AtRegistration: return "R"; case ClientState.PassedRegistration: return "O"; case ClientState.ReturningToWait: return "..."; case ClientState.AtDesk1: return "1"; case ClientState.AtDesk2: return "2"; case ClientState.AtLimitedZoneEntrance: return "..."; case ClientState.InsideLimitedZone: return "!"; case ClientState.Enraged: return "@"; case ClientState.Leaving: if(parent.reasonForLeaving == ClientPathfinding.LeaveReason.Angry) return "@"; if(parent.reasonForLeaving == ClientPathfinding.LeaveReason.CalmedDown) return ":("; if(parent.reasonForLeaving == ClientPathfinding.LeaveReason.Processed) return "O"; return "o"; default: return ""; }
    }
    ...
    private Color GetStateColor()
    {
        ...
        switch (state)
        {
            ...
            case ClientState.Leaving:
                ...
                if(parent.reasonForLeaving == ClientPathfinding.LeaveReason.CalmedDown) return Color.magenta; // ИЗМЕНЕНО ЗДЕСЬ
