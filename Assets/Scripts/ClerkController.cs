...
public class ClerkController : MonoBehaviour
{
    ...
    [Tooltip("Перетащите сюда префаб или объект рабочей станции (со скриптом ServicePoint)")]
    ...
    public string GetStatusInfo() { switch (currentState) { case ClerkState.Working: return $"Работает: {assignedServicePoint.name}"; case ClerkState.OnBreak: return "На перерыве (кухня)"; case ClerkState.AtToilet: return "На перерыве (туалет)"; case ClerkState.ReturningToWork: return $"Возвращается на работу: {assignedServicePoint.name}"; case ClerkState.GoingToBreak: return $"Идет на перерыв: {kitchenPoint.name}"; case ClerkState.GoingToToilet: return $"Идет в туалет: {staffToiletPoint.name}"; default: return currentState.ToString(); } }

    private void SetState(ClerkState newState) { if (currentState == newState) return; currentState = newState; LogCurrentState(); }
