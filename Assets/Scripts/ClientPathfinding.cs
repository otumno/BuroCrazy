...
public class ClientPathfinding : MonoBehaviour
{
    ...
    public void Initialize(GameObject rZ, GameObject wZ, GameObject tZ, GameObject d1, GameObject d2, GameObject cO, Waypoint eW) { totalClients++; stateMachine = gameObject.GetComponent<ClientStateMachine>(); movement = gameObject.GetComponent<ClientMovement>(); notification = gameObject.GetComponent<ClientNotification>(); queueManager = gameObject.GetComponent<ClientQueueManager>(); if (stateMachine == null || movement == null || notification == null || queueManager == null) { return; } stateMachine.Initialize(this); movement.Initialize(this); TextMeshPro tmp = GetComponentInChildren<TextMeshPro>(); if (tmp != null) notification.Initialize(this, tmp); queueManager.Initialize(this, rZ, wZ, tZ, d1, d2, cO, eW); totalPatienceTime = Random.Range(60f, 120f); if (spawnSound != null) AudioSource.PlayClipAtPoint(spawnSound, transform.position); }
    ...
    public void ForceLeave(LeaveReason reason = LeaveReason.CalmedDown) 
    {
        ...
        if (reason == LeaveReason.Angry && !ClientQueueManager.dissatisfiedClients.Contains(this))
        {
            ClientQueueManager.dissatisfiedClients.Add(this);
        }
