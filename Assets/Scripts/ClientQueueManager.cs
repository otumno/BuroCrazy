...
public class ClientQueueManager : MonoBehaviour
{
    ...
    void Update() 
    {
        if(!isRegistrationBusy) { CallNextClient(); }
    ...
    public Transform FindSeatForClient(ClientPathfinding client) 
    {
        ...
        if (freeSeat != null) { occupiedSeats[freeSeat] = client; if(standingClients.Contains(client)) standingClients.Remove(client); return freeSeat; } 
        else { if (!standingClients.Contains(client)) standingClients.Add(client); return null; }
    }
    ...
    public void OnClientLeavesWaitingZone(ClientPathfinding client) 
    {
        ...
        if (occupiedSeats.ContainsValue(client)) 
        {
            ...
            if (seatToFree != null) { occupiedSeats.Remove(seatToFree); FindAndAssignNearestStandingClient(seatToFree); }
        }
    }
    ...
    { 
        ...
        var nextInQueue = queue.Where(c => c.Key != null && (c.Key.stateMachine.GetCurrentState() == ClientState.AtWaitingArea || c.Key.stateMachine.GetCurrentState() == ClientState.SittingInWaitingArea)).OrderBy(kvp => kvp.Value).FirstOrDefault(); 
        ClientPathfinding nextClient = nextInQueue.Key; 
        if (nextClient != null) { isRegistrationBusy = true; clientBeingServed = nextClient; currentlyCalledNumber = nextInQueue.Value; noShowTimer = Time.time + noShowTimeout; nextClient.stateMachine.GetCalledToRegistrar(); return true; } 
        return false; 
    }
