...
public class ClientStateMachine : MonoBehaviour
{
    ...
    public IEnumerator MainLogicLoop()
    {
        ...
        while (true)
        {
            ...
            switch (currentState)
            {
                ...
                case ClientState.AtDesk1: yield return StartCoroutine(ServiceAtDesk(1)); break;
                case ClientState.AtDesk2: yield return StartCoroutine(ServiceAtDesk(2)); break;
                case ClientState.AtRegistration: bool hasNumber = parent.queueManager.HasQueueNumber(); if (!hasNumber) { if (ClientQueueManager.IsRegistrarBusy()) { parent.queueManager.JoinQueue(parent); SetGoal(parent.queueManager.GetWaitingWaypoint()); SetState(ClientState.ReturningToWait); break; } if (Random.value < 0.80f) { parent.queueManager.JoinQueue(parent); SetGoal(parent.queueManager.GetWaitingWaypoint()); SetState(ClientState.ReturningToWait); } else yield return StartCoroutine(ServiceAtTriage()); } else { if (ClientQueueManager.IsRegistrarBusyWithAnother(parent)) { yield return new WaitForSeconds(Random.Range(2f, 5f)); if (ClientQueueManager.IsRegistrarBusyWithAnother(parent)) SetGoal(parent.queueManager.GetWaitingWaypoint()); SetState(ClientState.ReturningToWait); } else yield return StartCoroutine(ServiceAtTriage()); } break;
    ...
    private IEnumerator FollowPath() { while (path.Count > 0) { if (parent.movement != null) { if (Vector2.Distance(transform.position, path.Peek().transform.position) < parent.movement.stoppingDistance) { path.Dequeue(); if (currentState != ClientState.Leaving && currentState != ClientState.Enraged && Random.value < forgetfulnessChancePerWaypoint) { SetState(ClientState.Confused); yield break; } if (path.Count == 0) break; } parent.movement.SetVelocity(((Vector2)path.Peek().transform.position - (Vector2)transform.position).normalized * parent.movement.moveSpeed); } yield return null; } if (parent.movement != null) parent.movement.SetVelocity(Vector2.zero); }
    ...
    private IEnumerator ServiceAtTriage() { parent.movement.SetLinearDrag(10f); personalSpaceCoroutine = StartCoroutine(ManagePositionInZone(parent.queueManager.GetRegistrationZone())); yield return new WaitForSeconds(Random.Range(parent.queueManager.minWaitTime, parent.queueManager.maxWaitTime)); StopPersonalSpaceCheck(); parent.movement.SetLinearDrag(1f); ClientQueueManager.ReleaseRegistrar(); Waypoint nextGoal = null; float choice = Random.value; if (choice < 0.40f && ClientQueueManager.IsDesk1Available()) nextGoal = parent.queueManager.GetDesk1Waypoint(); else if (choice < 0.80f && ClientQueueManager.IsDesk2Available()) nextGoal = parent.queueManager.GetDesk2Waypoint(); else if (choice < 0.90f) nextGoal = parent.queueManager.GetToiletWaypoint(); else if (choice < 0.98f) { parent.reasonForLeaving = ClientPathfinding.LeaveReason.Normal; nextGoal = parent.queueManager.GetExitWaypoint(); } else { parent.reasonForLeaving = ClientPathfinding.LeaveReason.Angry; ClientQueueManager.dissatisfiedClients.Add(parent); if (parent.dissatisfiedExitSound != null) AudioSource.PlayClipAtPoint(parent.dissatisfiedExitSound, transform.position); nextGoal = parent.queueManager.GetExitWaypoint(); } parent.queueManager.OnClientServedFromRegistrar(); SetGoal(nextGoal); if (nextGoal == parent.queueManager.GetExitWaypoint()) SetState(ClientState.Leaving); else SetState(ClientState.MovingToGoal); RecalculatePath(); }
    private IEnumerator ServiceAtDesk(int deskNum) { parent.movement.SetLinearDrag(10f); bool isMyDeskAvailable = (deskNum == 1) ? ClientQueueManager.IsDesk1Available() : ClientQueueManager.IsDesk2Available(); if (!isMyDeskAvailable) { SetGoal(parent.queueManager.GetWaitingWaypoint()); SetState(ClientState.ReturningToWait); RecalculatePath(); yield break; } bool isMyDeskBusy = (deskNum == 1) ? ClientQueueManager.IsDesk1Busy() : ClientQueueManager.IsDesk2Busy(); if (isMyDeskBusy) { yield return new WaitForSeconds(Random.Range(3f, 6f)); isMyDeskBusy = (deskNum == 1) ? ClientQueueManager.IsDesk1Busy() : ClientQueueManager.IsDesk2Busy(); if (isMyDeskBusy) { SetGoal(parent.queueManager.GetWaitingWaypoint()); SetState(ClientState.ReturningToWait); RecalculatePath(); yield break; } } ClientQueueManager.OccupyDesk(deskNum, parent); personalSpaceCoroutine = StartCoroutine(ManagePositionInZone((deskNum == 1) ? parent.queueManager.GetDesk1Zone() : parent.queueManager.GetDesk2Zone())); yield return new WaitForSeconds(Random.Range(parent.queueManager.minWaitTime, parent.queueManager.maxWaitTime)); StopPersonalSpaceCheck(); parent.movement.SetLinearDrag(1f); float choice = Random.value; if (choice < 0.25f) { Waypoint otherDesk = (deskNum == 1) ? parent.queueManager.GetDesk2Waypoint() : parent.queueManager.GetDesk1Waypoint(); SetGoal(otherDesk); SetState(ClientState.MovingToGoal); RecalculatePath(); } else { SetState(ClientState.PassedRegistration); } ClientQueueManager.ReleaseDesk(deskNum); }
    
    private IEnumerator ManagePositionInZone(GameObject zone)
    {
        float repositionSpeed = baseMoveSpeed * repositionSpeedMultiplier;
        Collider2D zoneCollider = zone.GetComponent<Collider2D>();
        if (zoneCollider == null) yield break;

        Vector2 targetPosition = Vector2.zero;
        for (int i = 0; i < 20; i++)
        {
            Vector2 randomOffset = new Vector2(Random.Range(-1f, 1f), Random.Range(-1f, 1f)) * (zoneCollider.bounds.size.x / 2.5f);
            targetPosition = (Vector2)zone.transform.position + randomOffset;
            if (zoneCollider.bounds.Contains(targetPosition) && Physics2D.OverlapCircle(targetPosition, personalSpaceRadius, LayerMask.GetMask("Client")) == null) break;
        }
        
        while (Vector2.Distance(transform.position, targetPosition) > 0.3f)
        {
            if (parent.movement == null) yield break;
            Vector2 direction = (targetPosition - (Vector2)transform.position).normalized;
            parent.movement.SetVelocity(direction * baseMoveSpeed * 0.5f);
            yield return null;
        }

        while (true)
        {
            ClientState s = GetCurrentState();
            bool shouldAvoidOthers = (s == ClientState.AtWaitingArea || s == ClientState.AtToilet || s == ClientState.InsideLimitedZone);
            if (!shouldAvoidOthers)
            {
            ...
            {
                moveVector += ((Vector2)zone.transform.position - (Vector2)transform.position).normalized;
            }
            
            Collider2D[] neighbors = Physics2D.OverlapCircleAll(transform.position, personalSpaceRadius, LayerMask.GetMask("Client"));
            if (neighbors.Length > 1)
            {
                foreach (var n in neighbors)
                {
                    if (n.gameObject == gameObject) continue;
                    moveVector += ((Vector2)transform.position - (Vector2)n.transform.position).normalized;
                }
            }
            if (moveVector.sqrMagnitude > 0.01f) parent.movement.SetVelocity(moveVector.normalized * repositionSpeed);
            else parent.movement.SetVelocity(Vector2.zero);
    ...
    private void BuildPathToGoal(Waypoint goal) { Waypoint[] availableWaypoints = FindObjectsByType<Waypoint>(FindObjectsSortMode.None).Where(wp => wp.type == Waypoint.WaypointType.General).ToArray(); if (availableWaypoints.Length == 0) return; Dictionary<Waypoint, float> distances = new Dictionary<Waypoint, float>(); Dictionary<Waypoint, Waypoint> previous = new Dictionary<Waypoint, Waypoint>(); PriorityQueue<Waypoint, float> queue = new PriorityQueue<Waypoint, float>(); foreach (var wp in availableWaypoints) { distances[wp] = float.MaxValue; previous[wp] = null; } Waypoint start = FindNearestWaypoint(transform.position, availableWaypoints); if (start != null) { distances[start] = 0; queue.Enqueue(start, 0); while (queue.Count > 0) { Waypoint current = queue.Dequeue(); if (current == goal) { ReconstructPath(previous, goal); return; } if (current.neighbors == null || current.neighbors.Count == 0) continue; foreach (var n in current.neighbors.Where(neighbor => neighbor.type == Waypoint.WaypointType.General)) { float newDist = distances[current] + Vector2.Distance(current.transform.position, n.transform.position); if (newDist < distances[n]) { distances[n] = newDist; previous[n] = current; queue.Enqueue(n, newDist); } } } } else { SetState(ClientState.Confused); return; } }
    ...
    private Waypoint FindNearestWaypoint(Vector2 p, Waypoint[] wps) { return wps.OrderBy(wp => Vector2.Distance(p, wp.transform.position)).FirstOrDefault(); }
