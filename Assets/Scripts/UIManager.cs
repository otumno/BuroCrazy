...
{ 
    ...
    { 
        ...
        if(nowServingText != null) 
        { 
            // Обращаемся к свойству через синглтон
            int number = ClientQueueManager.Instance.currentlyCalledNumber; 
            nowServingText.text = (number > 0) ? $"Вызывается: {number}" : "Регистратура"; 
        } 
    }
}